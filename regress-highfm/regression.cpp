    ...
    */
    ...
    while(1)
    {
        ...
        for(int index=0;index<ablity;index++)
        {
            ...
            for(int i=1;i<5;i++)
            {
                hx += (k[i] * vx[index][i]);//一次项
                hx += (k[i] * pow(vx[index][i],2) );//二次项
                hx += (k[i] * vx[index][i]*vx[index][(i+1)%5]);        //二次项
            }
            //2.得到偏差和与对应特征值的乘积
            for(int i=0;i<5;i++)
            {
                ...
                s_gap[i] += ((hx - database[y_line][index])*vx[index][i]*vx[index][(i+1)%5]);//二次项偏导数
            }
            //3.得到代价值，方便观察结果
